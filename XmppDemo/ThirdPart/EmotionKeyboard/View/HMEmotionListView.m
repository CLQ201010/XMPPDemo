...
- (void)setEmotions:(NSArray *)emotions
{
    ...
    // 设置总页数
    NSUInteger totalPages = (emotions.count + HMEmotionMaxCountPerPage - 1) / HMEmotionMaxCountPerPage;
    NSUInteger currentGridViewCount = self.scrollView.subviews.count;
...
//            gridView.backgroundColor = HMRandomColor;
        ...
        NSUInteger loc = i * HMEmotionMaxCountPerPage;
        NSUInteger len = HMEmotionMaxCountPerPage;
    ...
    // 隐藏后面的不需要用到的gridView
    for (NSUInteger i = totalPages; i<currentGridViewCount; i++) {
